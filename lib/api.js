...
module.exports = xiv => {
    ...
    router.get('/list/:page', (req, res) => {
        ...
        xiv.data.list(`BNpcName?page=${page}`).then((response) => {
            ...
            for(let i = 0; i < count; i++) {
                // Only allow entries that contain at least one alpha character
                // This purges the first entry "" and any Japanese-only entries
                if(/^[A-Za-z0-9'\s-,&#\.αβγ]+$/.test(response.Results[i].Name)){
    ...
    router.get('/:id', (req, res) => {
        xiv.data.get("BNpcName", req.params.id).then((response) => {
            ...
            let result;
            if(/^[A-Za-z0-9'\s-,&#\.αβγ]+$/.test(response.Name)){
                result = {
                    name: response.Name,
                    version: {
